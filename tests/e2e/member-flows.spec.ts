...
test("member visitors are redirected to sign-in before entering the app", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Sign in to continue your fitness journey"),
  ).toBeVisible();
});
...
test("approved member schedule preview opens cleanly and can enroll in a class", async ({
  ...
  await expect(
    page.getByTestId("schedule-mini-class-preview-open"),
  ).toBeVisible();
});
