...
## 2024-07-11 - [HIGH] Prevent Protocol Spoofing in Express Proxy Middleware

**Vulnerability:** The proxy middleware manually parsed and trusted the `x-forwarded-proto` header for protocol resolution, bypassing Express's `trust proxy` mechanism. This allowed attackers to spoof the protocol (SSRF/Host Header Injection risk).
**Learning:** When using `http-proxy-middleware` in an Express application, the `req` object provided to lifecycle hooks like `onProxyReq` is typed as Node's `http.IncomingMessage`. Manual parsing of forwarded headers ignores framework-level trust settings.
**Prevention:** Explicitly cast the request to the Express `Request` type and use `req.protocol`, which securely respects the application's `trust proxy` configuration, while retaining `req.headers.host` to safely preserve the port in local development environments.
