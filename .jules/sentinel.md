...
## 2024-05-20 - [CRITICAL] Fix DoS and Host Header Injection in Expo Server

**Vulnerability:** Uncaught `TypeError: Invalid URL` leading to Denial of Service (DoS) via malformed `host` headers, and Host Header Injection/SSRF via blind trust of `x-forwarded-host` in `artifacts/gymapp/server/serve.js`.
**Learning:** The `new URL()` constructor throws an exception if the URL is invalid. If an attacker provides an invalid `host` header (e.g. `%foo`), `new URL()` fails and crashes the Node.js server if unhandled. Additionally, reading `x-forwarded-host` in raw Node environments without a trusted proxy mechanism allows Host Header Injection.
**Prevention:** Always wrap `new URL()` calls in `try-catch` blocks when dealing with client-controlled inputs like headers. Never blindly trust `x-forwarded-host`; rely on the standard `host` header instead. When parsing headers like `x-forwarded-proto` that could be comma-separated, always split by comma and take the first trimmed value.
