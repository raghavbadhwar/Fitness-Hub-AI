...
## 2024-07-14 - SSRF and Protocol Spoofing via X-Forwarded Headers

**Vulnerability:** `serve.js` and `clerkProxyMiddleware.ts` manually parsed `x-forwarded-host` and `x-forwarded-proto` headers without validation.
**Learning:** Blindly trusting these headers can lead to Server-Side Request Forgery (SSRF) and protocol spoofing, as they can be easily manipulated by clients if not strictly validated or parsed securely by the framework's `trust proxy` mechanism.
**Prevention:** Always use framework-provided properties like `req.protocol` and `req.hostname` (which respect proxy settings), or validate standard `host` headers directly. Never blindly trust raw `x-forwarded-*` headers in standalone node servers.
