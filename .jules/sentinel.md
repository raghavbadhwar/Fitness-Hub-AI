...
## 2024-06-24 - Host Header Injection / SSRF in Proxy Middleware

**Vulnerability:** The clerk proxy middleware manually parsed `req.headers["x-forwarded-proto"]` and `req.headers.host` to construct the proxy URL. These client-controlled headers could be spoofed to perform Server-Side Request Forgery (SSRF) or Host Header Injection.
**Learning:** Manually parsing `x-forwarded-proto` or `host` headers directly bypasses Express's secure `trust proxy` mechanism.
**Prevention:** Always use `req.protocol` and `req.hostname` which safely respect proxy configurations. Combine `req.hostname` with the port extracted securely from `req.get("host")` using the `URL` parser rather than naive string splitting to support IPv6.
