...
## 2024-07-18 - Host Header Injection in Expo static server

**Vulnerability:** The standalone production server (`artifacts/gymapp/server/serve.js`) blindly trusted the `x-forwarded-host` header for the landing page URL construction.
**Learning:** In a raw Node.js server without a proxy trust mechanism (like Express's `trust proxy`), blindly trusting `x-forwarded-host` introduces Host Header Injection and SSRF vulnerabilities.
**Prevention:** Use the standard `host` header instead. When parsing headers like `x-forwarded-proto`, always account for comma-separated lists by splitting and extracting the first trimmed value. Also, add fallbacks for `req.headers.host` in `new URL()` constructions to prevent DoS via invalid URLs.
