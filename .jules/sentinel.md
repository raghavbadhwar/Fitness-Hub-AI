...
## 2025-02-12 - Host Header Injection & SSRF via x-forwarded-\* Headers

**Vulnerability:** The application blindly trusted `x-forwarded-host` and `x-forwarded-proto` headers in both Express proxy middleware and raw Node.js servers, allowing attackers to spoof the protocol and host, leading to Host Header Injection and SSRF.
**Learning:** In raw Node.js servers without proxy trust mechanisms, client-controlled proxy headers cannot be trusted. In Express, raw header access bypasses the framework's secure `trust proxy` configuration.
**Prevention:** Always use secure framework accessors (like `req.protocol` in Express) which respect proxy settings, or strict defaults in raw servers. When a port must be preserved for local dev in Express middlewares, use `req.get('host')`.
