...
## 2024-05-24 - Protocol Spoofing via Unvalidated Header

**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` and `host` headers from incoming requests to determine the proxy URL. Because these headers are client-controlled, a malicious actor could send a spoofed header to bypass proxy logic, trigger SSRF-like behavior, or cause the proxy to rewrite endpoints to unintended URLs.
**Learning:** Manually parsing `x-forwarded-proto` and `host` directly from `req.headers` skips the framework's secure handling of proxy trust. When an Express application is running behind a trusted proxy, the framework needs to be explicitly configured using `app.set("trust proxy", ...)` to securely parse these headers.
**Prevention:** Always use `req.protocol` and `req.get("host")` in Express rather than reading `req.headers["x-forwarded-proto"]` or `req.headers.host` directly. This ensures Express properly respects proxy trust configurations.
