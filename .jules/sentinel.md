...
## 2024-07-13 - Protocol Spoofing via Unvalidated Header

**Vulnerability:** The clerk proxy middleware manually parsed `x-forwarded-proto` directly from headers, allowing attackers to spoof the protocol scheme and bypass Express's secure proxy trust mechanism.
**Learning:** Manually parsing proxy headers like `x-forwarded-proto` or `x-forwarded-host` bypasses Express's `trust proxy` setting, enabling SSRF or spoofing attacks. When running behind a trusted proxy, always rely on framework accessors that respect trust boundaries (like `req.protocol`).
**Prevention:** Use `req.protocol` for secure protocol resolution and strictly validate it (e.g. `protocol === 'http' ? 'http' : 'https'`). For host resolution, retain `req.get('host')` if the port must be preserved for local development, as `req.hostname` strips it.
