...
**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests to determine the protocol. Because this header is client-controlled, a malicious actor could send a spoofed `x-forwarded-proto` header to bypass intended protocol logic or spoof proxy URLs.
**Learning:** Manually parsing `x-forwarded-proto` directly from `req.headers` skips the framework's secure handling of proxy trust. When an Express application is running behind a trusted proxy, the framework needs to be explicitly configured using `app.set("trust proxy", ...)` to securely parse the protocol.
**Prevention:** Always use `req.protocol` in Express (or the equivalent secure accessor in other frameworks) rather than reading `req.headers["x-forwarded-proto"]` directly. Provide a fallback (e.g., `req.protocol || "https"`) to prevent regressions if trust proxy is misconfigured.
