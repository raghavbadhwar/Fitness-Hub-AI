...
## 2024-07-27 - Uncaught Exception DoS & SSRF via Host Headers

**Vulnerability:** Raw Node.js servers crashed with `TypeError: Invalid URL` when passing malicious `Host` headers to `new URL()`. Additionally, trusting `x-forwarded-host` blindly introduced SSRF and Host Header Injection risks.
**Learning:** Fallbacks like `req.headers.host || 'localhost'` are insufficient because `new URL()` strictly validates inputs. Invalid characters (e.g., `%foo`) will bypass the fallback and crash the server. `x-forwarded-host` should not be trusted outside of Express's `trust proxy` mechanism.
**Prevention:** Always wrap `new URL()` construction in a `try-catch` block when passing client-controlled headers to handle invalid inputs safely. Never trust `x-forwarded-host` in raw servers; stick to the standard `host` header.
