...
**Vulnerability:** The clerk proxy middleware manually extracted the protocol from `req.headers["x-forwarded-proto"]` instead of using Express's `req.protocol`. This allowed attackers to spoof the protocol and bypass proxy trust settings, which can lead to improper routing or security checks.
**Learning:** Manually parsing `x-forwarded-proto` bypasses Express's `trust proxy` configuration. Always use `req.protocol` which securely respects these configurations, ensuring the application interprets the proxy headers correctly.
**Prevention:** Rely on `req.protocol` instead of manually reading `x-forwarded-proto` from headers. Provide a secure fallback like `req.protocol || "https"` for environments where proxy trust might be misconfigured or disabled.
