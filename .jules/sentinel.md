...
## 2024-05-15 - Protocol Spoofing in Proxy Middleware

**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests. Because this header is client-controlled, a malicious actor could send a spoofed `x-forwarded-proto` header to bypass protocol-based logic or poison downstream proxy URLs.
**Learning:** Manually parsing `x-forwarded-proto` directly from `req.headers` skips Express's secure handling of proxy trust. `req.protocol` should be used instead, as it respects the application's `trust proxy` configuration.
**Prevention:** Always use `req.protocol` securely in Express, but retain `req.get('host')` for host resolution to safely preserve the port in local development environments.
