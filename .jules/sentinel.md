...
**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests to determine the protocol. Because this header is client-controlled, a malicious actor could send a spoofed header.
**Learning:** Manually parsing `x-forwarded-proto` directly from `req.headers` skips the framework's secure handling of proxy trust, similar to IP spoofing.
**Prevention:** Use `req.protocol` to securely resolve the protocol, which respects the Express `trust proxy` setting, while continuing to use `req.get("host")` to safely resolve the host while preserving the port for local environments.
