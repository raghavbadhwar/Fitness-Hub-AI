...
## 2024-05-15 - Host Header Injection and SSRF in Clerk Proxy Middleware

**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` and `host` headers from `req.headers` to construct the `Clerk-Proxy-Url`. Since these are client-controlled, it allowed spoofing the proxy URL and bypassed the framework's secure proxy configuration.
**Learning:** Blindly trusting `x-forwarded-host` or manually parsing `host` bypasses Express's `trust proxy` mechanism.
**Prevention:** In Express, securely extract the host and port by using `req.hostname` (which strips the port but respects `trust proxy`) and carefully parsing the port from `req.get('x-forwarded-host')` or `req.get('host')` with a fallback, ensuring the hostname matches `req.hostname`. Always wrap `URL` parsing in `try...catch` to avoid DoS via invalid URLs. Explicitly cast `http.IncomingMessage` to Express `Request` when in `http-proxy-middleware` hooks.
