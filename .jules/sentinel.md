...
## 2024-05-15 - Protocol Spoofing via x-forwarded-proto

**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests to determine the request protocol. Since this header is client-controlled, an attacker could send a spoofed `x-forwarded-proto` header, leading to protocol spoofing and potentially incorrect URL generation for proxy redirects.
**Learning:** Blindly trusting `x-forwarded-proto` by parsing it manually from `req.headers` ignores Express's secure proxy trust handling. To securely resolve the protocol behind a reverse proxy, the application must rely on the framework's internal trust mechanisms instead of implementing custom parsing logic.
**Prevention:** Use `req.protocol` in Express to resolve the request protocol. This property correctly respects the `trust proxy` configuration, safely returning the true protocol while rejecting spoofed headers from untrusted sources.
