...
**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests to determine the protocol. Because this header is client-controlled, an attacker could spoof the protocol, bypassing intended security checks.
**Learning:** Parsing `x-forwarded-proto` directly from `req.headers` skips the framework's secure handling of proxy trust, similar to IP spoofing. Express securely handles protocol evaluation through `req.protocol` when correctly configured to trust the proxy.
**Prevention:** Always use `req.protocol` in Express to determine the protocol securely rather than reading `req.headers["x-forwarded-proto"]` directly.
