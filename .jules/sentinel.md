...
**Vulnerability:** The API server's clerk proxy middleware manually parsed the `x-forwarded-proto` header from incoming requests to determine the protocol. Because this header is client-controlled, a malicious actor could send a spoofed `x-forwarded-proto` header, leading to SSRF or protocol spoofing vulnerabilities.
**Learning:** Manually parsing `x-forwarded-proto` directly from `req.headers` skips the framework's secure handling of proxy trust. When an Express application is running behind a trusted proxy, the framework should be relied upon using `req.protocol` to securely parse the X-Forwarded-Proto header.
**Prevention:** Always use `req.protocol` (cast `req` to Express `Request` type) to determine the protocol instead of reading `req.headers["x-forwarded-proto"]` directly. Ensure the application's `trust proxy` setting is correctly configured.
