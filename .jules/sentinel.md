...
## 2024-07-30 - Fix DoS and Host Header Injection in Expo Server

**Vulnerability:** Uncaught `TypeError: Invalid URL` during `new URL()` parsing of malformed host headers leading to Denial of Service, and blind trust of `x-forwarded-host` leading to Host Header Injection.
**Learning:** Raw Node.js servers without proper `trust proxy` mechanisms cannot blindly trust `x-forwarded-host` headers. Furthermore, passing untrusted input directly to `new URL()` can crash the process if the input is malformed (e.g., `%foo`).
**Prevention:** Always wrap `new URL()` parsing in a `try-catch` block when handling untrusted or malformed input. Only trust `x-forwarded-*` headers when properly configuring a proxy trust layer.
