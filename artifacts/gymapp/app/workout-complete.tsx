  ...
  const completedSets =
    session?.exercises?.reduce(
      (total: number, ex: WorkoutExercise) =>
        total + ex.sets.filter((s: ExerciseSet) => s.completed).length,
      0,
    ) ?? 0;
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Animated.View
          style={[styles.heroSection, { opacity: opacityAnim, transform: [{ scale: scaleAnim }] }]}
        >
          ...
          <Text style={[styles.sessionName, { color: colors.mutedForeground }]}>
            {session.name}
          </Text>
        </Animated.View>
          ...
          <Animated.View style={[styles.prSection, { transform: [{ scale: prScaleAnim }] }]}>
            ...
            {newPRs.map((pr) => (
              <View
                ...
                style={[
                  styles.prCard,
                  { backgroundColor: colors.card, borderColor: colors.warning + "30" },
                ]}
              >
                <Text style={styles.prStar}>⭐</Text>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.prName, { color: colors.text }]}>{pr.name}</Text>
                  <Text style={[styles.prWeight, { color: colors.warning }]}>
