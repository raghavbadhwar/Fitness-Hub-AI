...
export default function WorkoutScreen() {
  ...
  const aiCoachSubtitle =
    isMemberView && behaviorProfile.workoutsLast30Days > 0
      ? behaviorProfile.coachingFocus
      : "Personalized plan based on your history & goals";
  const myPlansSummary =
    behaviorProfile.workoutsLast30Days > 0
      ? `${behaviorProfile.consistencyLabel === "locked-in" ? "Keep your best routines ready." : "Save routines you can repeat without overthinking."} Best flow so far: ${behaviorProfile.preferredWorkoutWindow === "varied" ? "varied workout times" : `${behaviorProfile.preferredWorkoutWindow} sessions`}.`
      : "Save one custom routine so your next workout is always one tap away.";
  ...
  const handleDeleteSavedPlan = useCallback(
    (plan: SavedWorkoutPlan) => {
      Alert.alert(
        "Delete plan",
        `Remove "${plan.name}" from My Plans?`,
        [
