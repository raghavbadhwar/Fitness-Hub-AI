...
describe("clerkProxyMiddleware", () => {
  ...
  it("handles missing req.headers gracefully in on.proxyReq", () => {
    ...
    // Should use express req.protocol (or fallback to https if undefined) and empty host if host missing
    assert.equal(setHeaders.get("Clerk-Proxy-Url"), `http://${CLERK_PROXY_PATH}`);
    assert.equal(setHeaders.get("Clerk-Secret-Key"), "test_secret_key");
