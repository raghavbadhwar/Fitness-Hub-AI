  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        const host = expressReq.get ? expressReq.get("host") : expressReq.headers.host;
        const finalHost = host || expressReq.headers.host || "";
        const proxyUrl = `${protocol}://${finalHost}${CLERK_PROXY_PATH}`;
