    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        try {
          ...
          if (xForwardedHost) {
            const parsedXFH = xForwardedHost.split(",")[0].trim();
            const url = new URL(`${protocol}://${parsedXFH}`);
            if (url.hostname === expressReq.hostname) {
              host = url.host;
            }
          }
        } catch {
          // Ignore parsing errors and fallback to original host
        }
