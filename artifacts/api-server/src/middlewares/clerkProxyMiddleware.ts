  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        try {
          const targetHost = expressReq.get("x-forwarded-host") || expressReq.get("host") || "";
          if (targetHost) {
            const parsed = new URL(`http://${targetHost}`);
            if (parsed.hostname === expressReq.hostname) {
              host = targetHost;
            }
          }
        } catch {
          // Fallback to expressReq.hostname
        }
