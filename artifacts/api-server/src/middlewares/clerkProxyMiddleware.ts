  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        try {
          if (rawHost) {
            port = new URL(`http://${rawHost}`).port;
          }
        } catch {
          // Ignore DoS from malformed host
        }

        const safeHostname = expressReq.hostname || "localhost";
        const hostWithPort = port ? `${safeHostname}:${port}` : safeHostname;
