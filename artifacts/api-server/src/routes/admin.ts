...
router.get("/dashboard", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Optimize dashboard stats with single-pass O(N) accumulation to prevent excessive array allocations
    let totalClassesThisWeek = 0;
    let totalEnrollments = 0;
    const categoryCounts: Record<string, number> = {};
    const classCountsByDate: Record<string, number> = {};
    ...
    for (const c of allClasses) {
      ...
      classCountsByDate[c.date] = (classCountsByDate[c.date] ?? 0) + 1;
    }
    ...
    try {
      // Use reduce instead of filter().length to avoid allocating intermediate arrays
      totalActiveMembers = (
