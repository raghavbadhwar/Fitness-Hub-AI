...
router.get("/dashboard", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Use parallel Drizzle aggregations instead of full table fetching
    const [
      [{ totalEnrollmentsRaw }],
      thisWeekClasses,
      categoryGroups
    ] = await Promise.all([
      db.select({ totalEnrollmentsRaw: sum(gymClassesTable.enrolledCount) })
        .from(gymClassesTable)
        .where(eq(gymClassesTable.gymId, access.gymId)),
      db.select({ date: gymClassesTable.date })
        .from(gymClassesTable)
        .where(and(
      ...
      db.select({ category: gymClassesTable.category, count: count() })
        .from(gymClassesTable)
        .where(eq(gymClassesTable.gymId, access.gymId))
    ...
    const totalEnrollments = Number(totalEnrollmentsRaw) || 0;
    const mostPopularCategory = categoryGroups[0]?.category ?? "None";
