...
router.get("/dashboard", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization:
    // Offloaded O(N) in-memory array aggregations to the DB.
    // Replaced fetching all classes into memory with `sum` and `count` / `groupBy` queries.
    // Reduces memory bloat and speeds up the endpoint for gyms with large class histories.
    const [enrollmentsResult, popularCategoryResult, thisWeekClasses] = await Promise.all([
      db
    ...
    const totalEnrollments = Number(enrollmentsResult[0]?.total ?? 0);
    const mostPopularCategory = popularCategoryResult[0]?.category ?? "None";
