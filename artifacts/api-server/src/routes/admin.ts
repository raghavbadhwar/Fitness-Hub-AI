...
import { and, eq, sum, count, gte, lte, sql, desc } from "drizzle-orm";
...
router.get("/dashboard", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // BOLT: Optimized dashboard stats by replacing in-memory operations over all historical
    // classes with database-level aggregations and targeted queries
    const [enrollmentsResult, categoryResult, thisWeekClasses] = await Promise.all([
      db
    ...
    const mostPopularCategory = categoryResult[0]?.category ?? "None";
